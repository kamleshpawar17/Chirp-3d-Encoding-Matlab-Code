...
% --- Load CS Image Data ---- %
load('./data/Swi/CS_Recon_Acc3_F.mat')
I = hdf5_to_cplxmat('./data/Swi/Image_Fourier.h5');
nstart = 145; nblck = 20;
I = I(:, nstart:nstart+nblck-1, :, :);
I = permute(I, [1, 3, 2]);
Irec_F = Irec_F(:, :, nstart:nstart+nblck-1);

Irec_F = Irec_F/max(abs(Irec_F(:)));
I = I/max(abs(I(:)));

% ---- Compute SWI and MIP ---- %
[~, ~, swi_n_cs, ~] = SWI_Recon_3D(Irec_F);
...
figure; 
subplot(2,2,1); imshow(I_n_cs,[],'InitialMagnification', scale); title('CS Fourier MIP negative')
subplot(2,2,2); imshow(I_n,[],'InitialMagnification', scale); title('Fourier MIP negative')
